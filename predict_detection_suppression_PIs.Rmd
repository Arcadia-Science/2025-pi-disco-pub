...
source("~/Downloads/trait_mapping_functions.R")
library(tidyverse)
library(pbapply)
library(ape)
library(familiar)
...
protease_inhibitors_high <- read_tsv("PI_orthogroups_high_qual_06112023.tsv")
protease_inhibitors_low <-read_tsv("PI_orthogroups_low_qual_06112023.tsv")
...
# These data are a list of matrices (one per-gene family) in a format that is
# not particularly useful for making comparisons across gene families:
...
# Okay, these data look much better and are actually in a more useful format:
...
dat <- read_tsv("chelicerate-species-metadata.txt")
